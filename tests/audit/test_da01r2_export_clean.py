...
def test_da01r2_export_clean_has_no_forbidden_payloads():
    root_value = os.environ.get("DA01R2_EXPORT_CLEAN_ROOT")
    if not root_value:
        pytest.skip("DA01R2_EXPORT_CLEAN_ROOT not set")
    root = Path(root_value)
    scan = json.loads((root / "export_clean_path_scan.json").read_text(encoding="utf-8"))
    assert scan["violations"] == []
    package = root / "paper10_da3_da01r2_18classic_pack.zip"
