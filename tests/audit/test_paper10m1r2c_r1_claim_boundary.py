...
def test_claim_boundary_forbids_universal_superiority_and_old_m1r2c_yaw_claims():
    text = (stage_root() / "10_CLAIM_BOUNDARY" / "PAPER10M1R2C_R1_FORBIDDEN_CLAIMS.md").read_text(
        encoding="utf-8"
    )
...
def test_allowed_claims_are_bounded():
    with (stage_root() / "10_CLAIM_BOUNDARY" / "PAPER10M1R2C_R1_ALLOWED_CLAIMS.csv").open(
