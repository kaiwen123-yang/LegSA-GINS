...
def test_matrix_runner_completes_status_diagnostic_rows(tmp_path):
    ...
    assert len(result["row_results"]) == 18
    assert all(row["method_mode"] == "status_diagnostic" for row in result["row_results"])
    assert all(row["terminal_status"] == "COMPLETED_EVALUABLE_DIAGNOSTIC_FALLBACK" for row in result["row_results"])
