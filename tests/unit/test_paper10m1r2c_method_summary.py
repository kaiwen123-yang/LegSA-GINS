...
def test_method_summary_has_four_modes_and_541_completed_each():
    rows = read_csv(stage_root() / "06_METHOD_SUMMARIES" / "PAPER10M1R2C_METHOD_LEVEL_SUMMARY.csv")
    assert len(rows) == 4
    for row in rows:
...
def test_method_comparison_table_is_bounded():
    rows = read_csv(stage_root() / "06_METHOD_SUMMARIES" / "PAPER10M1R2C_METHOD_MODE_COMPARISON_TABLE.csv")
    assert rows
