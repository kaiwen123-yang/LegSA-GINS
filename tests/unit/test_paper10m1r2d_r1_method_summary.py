...
def test_method_summary_has_nine_methods_and_541_completed_each():
    rows = read_csv(stage_root() / "07_METHOD_SUMMARIES" / "PAPER10M1R2D_R1_METHOD_LEVEL_SUMMARY.csv")
    assert len(rows) == 9
    for row in rows:
...
def test_method_comparison_table_is_bounded():
    rows = read_csv(stage_root() / "07_METHOD_SUMMARIES" / "PAPER10M1R2D_R1_METHOD_COMPARISON_TABLE.csv")
    assert rows
