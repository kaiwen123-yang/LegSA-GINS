...
import numpy as np
import pytest
from legsa_gins.paper_rebuild.clean5_parity.runtime import bind_config,expected_counts
...
def test_18_column_eligibility_and_profile_aware_attempts(tmp_path):
    ...
    imu=np.zeros((3,7));imu[:,0]=[65.9,66.005,340.1]
    ...
    assert counts['position_update_count']==3
    assert counts['receiver_velocity_update_count']==2
    assert counts['dual_yaw_attempt_count']==2
