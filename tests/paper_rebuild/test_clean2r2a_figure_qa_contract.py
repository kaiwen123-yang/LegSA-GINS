...
def test_source_aware_panel_never_draws_a_false_reject_and_requires_visual_review() -> None:
    ...
    assert "CLEAN2R2A_FIGURE_MACHINE_QA.json" in source
    assert "CLEAN2R2A_FIGURE_VISUAL_REVIEW.json" in source
...
def _review_fixture(tmp_path: Path, *, overlap: bool) -> tuple[Path, Path]:
    ...
    with (figures / "CLEAN2R2A_FIGURE_MANIFEST.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(manifest_rows[0])); writer.writeheader(); writer.writerows(manifest_rows)
    (figures / "CLEAN2R2A_FIGURE_MACHINE_QA.json").write_text(
...
def test_visual_review_is_decision_backed_and_fails_on_overlap(tmp_path: Path) -> None:
    ...
    payload = json.loads(
        (failed_stage / "10_DIAGNOSTIC_FIGURES/CLEAN2R2A_FIGURE_RENDER_QA.json").read_text(encoding="utf-8")
    )
