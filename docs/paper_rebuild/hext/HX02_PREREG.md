...
结果阶段补记（全部运行结束后写入，不改变任何登记规则）：

- 实际停止点与指令写法不同。指令写的是“等当前原生运行（EXT01 BY2）完成并归档后、下一次原生启动前”停止；实际停在 GINav BY2 评估完成后（12:35:42.707Z 记 EVALUATED）、BY2 相对位姿评估启动前（12:35:42.734Z 冻结）。
- 原因：Hartley 两支为批末成对评估，指令假设的边界不存在。批内先依次跑完 8 个原生运行，控制器在批末才评估 Hartley 两支，其间没有原生启动。修正到达时 BY2 批已进行到 GINav 原生；若等到下一次原生启动（BY2H EXT01），BY2 的相对位姿评估会先按旧规则执行。停止时未中断任何原生运行。
- 续作：12:45:00Z 以提交 `88594ef` 续作，冻结清单 53 个文件全部一致。
- 停止过程中有一次无后果的操作失误（停止程序第一次抓错 PID），记于 `$HX02/00_CONTROL/INCIDENTS.jsonl` 与 HX02_RESULTS.md 的“事故”一节。
