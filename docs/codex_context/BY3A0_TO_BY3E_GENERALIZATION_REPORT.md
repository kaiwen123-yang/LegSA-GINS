...
BY3C generated candidate Go2 IMU and GNSS-status inputs in the BY2-compatible schema, but the runtime configs keep execution disabled until provider and runner gates are explicitly accepted. BY3D and BY3E did not run. No solver/evaluator output, BY3 normal metrics, BY3 degradation/full matrix, random generation, degraded-input generation, final_v23 modification, algorithm math change, feedback-policy change, or paper claim was produced.
