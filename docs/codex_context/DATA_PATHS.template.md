...
- XB1 poor-GNSS generalization outputs use `<XB1_OUTPUT_ROOT>`, `<XB1_STAGE_ROOT>`, `<XB1A1_STAGE_ROOT>`, `<XB1_FULL_MATRIX_ROOT>`, `<XB1A1_NORMAL_GATE_ROOT>`, and `<XB1_EXPORT_CLEAN_ROOT>`. XB1 receiver/body sources use `<XB1_RECEIVER_ROOT>` and `<XB1_BODY_SOURCE>`.
