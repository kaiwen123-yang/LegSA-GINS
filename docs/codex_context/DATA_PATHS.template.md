...
- BY3 generalization outputs use the `BY3_*` aliases, including `<BY3A4A_STAGE_ROOT>` for the lateral yaw repair memory-lock stage, `<BY3A4C_STAGE_ROOT>` for the git-history yaw-reference reconstruction stage, `<BY3A5B_STAGE_ROOT>` for the A1 dual-diff yaw-input repair stage, `<BY3A6_STAGE_ROOT>` for the trace-truth/initatt/gate forensic stage, `<BY3A7_STAGE_ROOT>` for the A1 yaw dynamic-quality / IMU gate repair stage, and `<BY3A8_STAGE_ROOT>` for the yaw error-budget safe-repair stage.
