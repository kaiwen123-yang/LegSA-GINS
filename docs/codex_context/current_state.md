# Current State - GEN1 BY2-BY3 Generalization And XB1 Poor-GNSS Bootstrap
...
- Current poor-GNSS stage: `XB1A0_TO_XB1E_POOR_GNSS_GENERALIZATION_CONTEXT_QUALITY_AUDIT_ALIGNMENT_AND_NORMAL_RUN`.
...
- BY3C1/BY3Y1 reviewed the existing BY3C Batch0-Batch3 evidence only. It audited result integrity, generated BY3 three-scheme position/up tables and existing-metric figures, compared overlapping BY2/BY3 families, wrote family reviews, and produced the BY3Y1 yaw diagnostic explanation from BY3A5B-BY3A8 evidence. It did not run solvers, evaluators, degraded-input generation, random generation, parameter retuning, or paper claims.
- GEN1 reviewed existing BY2/BY3 metric and figure evidence only. It built BY2 and BY3 metric inventories with 213 rows each across 71 comparable case units, created canonical mapping and cross-dataset tables, generated cross-dataset figures from existing metrics only, inventoried existing BY3 figures, copied 874 unique nonempty figure files into a copy-only organized BY3 figure view, created export-clean material, and synced local Obsidian notes. It did not run solvers, evaluators, degraded-input generation, random generation, parameter retuning, or paper claims.
- XB1 / PG1_20260105_122513 completed the first poor-GNSS repeated-experiment bootstrap. It generated literature-backed quality criteria, receiver/body inventory, GNSS quality profile, kick-event alignment, body IMU from `<XB1_BODY_SOURCE>`, Go2 priors, input/provider reports, real GNSS quality figures, case review, export-clean material, and local Obsidian notes.
- XB1 GNSS quality is classified `severe`. Alignment passed without trace tuning. A1 short-baseline yaw is blocked by invalid geometry, and status long-baseline `rel_pos_n/e/d` plus HDT remain forbidden as mainline yaw sources.
- XB1 normal solver/evaluator execution did not run. The gate blockers are `inputs_not_ready_for_normal`, `A1_short_baseline_yaw_gate_blocked`, and `providers_not_ready_for_LegSA_full`; Raw Doppler provider materialization failed because the RTKLIB Doppler helper compile tool was unavailable in the current Windows runtime.
...
XB1 is the active poor-GNSS bootstrap decision. The final decision is `XB1_severe_GNSS_quality_mainline_limited`: quality/input/provider gates blocked normal solvers before execution, no normal metrics exist, quality-aware branch planning is ready only after human review, and paper claims remain disabled.
...
recommended_XB1_next_stage=human_review_XB1_then_repair_provider_or_plan_quality_aware_branch
```
...
BY3C1/BY3Y1 is the current BY3 review state. It completed a reporting-only review of existing BY3C Batch0-Batch3 metrics, existing BY3A8 yaw evidence, and existing BY2 N9C0D/N9C2B comparison material. Result integrity passed for 71 BY3 case units and 213 final metric rows. It generated BY3 three-scheme position/up tables, family reviews, BY2-vs-BY3 trend classifications, existing-metric review figures, case-review packages, an export-clean package, and Obsidian notes. It did not run solvers, evaluators, degraded-input generation, random generation, parameter retuning, or paper claims.

The BY3C1 trend classification is conservative: B downsample is `generalizes_consistently`; normal, A outage, E position std, C position noise, and D position spike are `same_order_no_clear_advantage` or mixed. BY3Y1 explains yaw as diagnostic-only: wrong-source yaw, stale initatt, and moving-segment IMU bias were repaired, while A1 observation quality remains the limiting factor and selected feedback worsens normal diagnostic yaw slightly relative to stage1.

Current decision: `BY3C1_position_up_review_and_yaw_diagnostic_complete`; `ready_for_BY3D_or_other_dataset_planning=true_after_human_review`; `yaw_claim_scope=diagnostic_only`; `ready_for_paper_claims=false`.
