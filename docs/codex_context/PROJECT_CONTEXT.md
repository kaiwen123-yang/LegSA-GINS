...
- Current paper evidence review root: `<PAPER0_STAGE_ROOT>`.
...
- PG_QA0 completed a design-only quality-aware fallback package. It keeps BY2/BY3 position-up generalization as the main proven line, keeps BY3 yaw diagnostic-only, treats PG1-PG4 severe GNSS as motivation, and defines the separate future `LegSA_QA_Fallback_EKF` candidate without implementing or validating it.
- PAPER0 completed a paper-facing evidence package after PG_QA0. It organizes BY2 as the full-metric main dataset, BY3 as independent position/up generalization with diagnostic-only yaw, PG1-PG4 as severe-GNSS boundary/QA motivation, and PG_QA0 as a design extension. It generated table drafts, figure recommendations, narrative/claim-boundary material, journal-positioning notes, and missing-work decisions from existing evidence only.
- Current active-FGO recommended next stage: `implement_active_fgo_backend_or_reframe_scope`.
- Current paper recommended next stage: `PAPER1_MANUSCRIPT_EXPERIMENT_SECTION_DRAFT`. QA1 remains optional for a stronger secondary-contribution route, not required before starting the near-term Option B manuscript.
...
PG_QA0 is the current poor-GNSS design boundary. PG_MULTI_A0 remains the source/runnability evidence import, and all PG1-PG4 A1 invalidity decisions still require status relpos dual-difference evidence rather than single rel_pos, long-relpos, HDT, absolute-LLH-only fallback, trace, final_v23 output, or solver output. QA0 does not authorize paper claims, frozen dual-yaw mainline normal runs, degradation, random arrays, retuning, quality-aware implementation, trace-tuned thresholds, or relabeling `LegSA_full_EKF`.

PAPER0 is the current paper-facing evidence boundary. It supports starting manuscript experiment-section drafting from existing BY2/BY3 evidence with PG/QA as limitation/design extension. It does not authorize final paper claims, final figures, solvers, evaluators, degraded-input generation, random arrays, QA1 implementation, retuning, metric alteration, PG performance claims, BY3 yaw main claims, or comprehensive final_v23 superiority claims.
