...
- BY3A5B superseded BY3A5's HDT replacement policy, regenerated BY3 mainline dual yaw from GNSS1/GNSS2 A1_dual_diff short-baseline positions with BY2 lateral conversion and fixed_1p5 yaw_std, and completed a BY3 normal-only rerun. Official yaw remains a reference/evaluator issue, so yaw degradation claims remain false.
- Current BY3 recommended next stage: `human_review_yaw_reference_or_position_only_BY3B`.
...
BY3 degradation planning after BY3A5B is position/up-only. BY3 yaw input is repaired from A1_dual_diff short baseline, but the official yaw reference/evaluator remains unresolved; BY3A3/BY3A4A/BY3A5 yaw metrics remain historical bad-input or invalid-reference evidence and must not support yaw degradation, final_v23 outperformance, or paper claims.
