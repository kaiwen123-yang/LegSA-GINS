...
- Current BY3 recommended next stage: `BY3B_DEGRADATION_MATRIX_PLANNING_AND_PRECHECK` after human review.
...
BY3 degradation planning after BY3A7 is ready only after human review. BY3 yaw input remains A1_dual_diff short baseline, stale first-row initatt was repaired in BY3A6, and BY3A7 repaired a BY3-local IMU preprocessing bias bug without changing yaw gates or tuning parameters. BY3A3/BY3A4A/BY3A5/BY3A5B/BY3A6 yaw metrics remain historical bad-input, invalid-reference, or pre-BY3A7 evidence and must not support paper claims or final_v23 outperformance.
