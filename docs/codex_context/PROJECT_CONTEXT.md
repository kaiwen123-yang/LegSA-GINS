...
- BY3A6 validated the BY3 trace truth/evaluator/base_time chain, confirmed processed trace lat/lon are unsafe for blind evaluation, repaired stale first-row initatt for stage1/LegSA, and completed a BY3 normal-only rerun. Yaw still fails, so yaw degradation claims remain false.
- Current BY3 recommended next stage: `human_review_yaw_issue_or_position_only_BY3B`.
...
BY3 degradation planning after BY3A6 is blocked pending human review. BY3 yaw input is repaired from A1_dual_diff short baseline and stale first-row initatt was repaired, but the post-repair yaw chain still fails; BY3A3/BY3A4A/BY3A5/BY3A5B yaw metrics remain historical bad-input, invalid-reference, or pre-repair evidence and must not support yaw degradation, final_v23 outperformance, or paper claims.
