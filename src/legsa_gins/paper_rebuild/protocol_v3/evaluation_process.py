"""Frozen evaluator transport, with a passive v3 matched-Truth observer.

Only the observer module path and optional capture request differ from the
frozen transport; evaluator file, argv, consistency policy and math are exact.
"""
...
def evaluate(*, evaluator, trace, nav, std, outdir, base_time, window,
             ...
             consistency_policy=None, measure_resources=False, export_matched_truth=False):
    ...
    config = {"evaluator": str(evaluator), "evaluator_sha256": EVALUATOR_SHA256,
              ...
              "v3_export_matched_truth": bool(export_matched_truth)}
    ...
    if instrument:
        environment["PYTHONPATH"] = os.pathsep.join((str(code_root / "scripts/paper_rebuild/v3_evaluator_observer"), str(code_root / "src")))
    ...
    if export_matched_truth:
        for name in ("MATCHED_TRAJECTORY.csv", "MATCHED_TRAJECTORY_MANIFEST.json"):
            if not (outdir / name).is_file():
                raise RuntimeError("V3 evaluator observer missing " + name)
