"""Non-overwriting v2.1 export of 28 figures; MFIG21 remains in its original archive."""
...
from .protocol_v21_data import Bundle, EvidenceUnavailable, sha256_file
from .protocol_v21_figures import FIGURES
...
def _render_figure(bundle, out, figure_id, trace_sha256, head, source_files, replace, repair_manifests=False):
    ...
    entry = {"figure_id": figure_id, "title": title, "kind": kind, "protocol_version": "v2.1",
...
def render(package, out_dir, *, trace_path=None, trace_sha256=None, figures=None, replace=False, workers=16, repair_manifests=False):
    ...
    if {"v1_prereg", "v2"}.intersection(out.parts):
        raise ValueError("The v1/v2 figure archives are immutable")
    ...
    source_files = {"publication/"+p.name: sha256_file(p) for p in Path(__file__).parent.glob("protocol_v*.py")}
    ...
    for figure_id in FIGURES:
        ...
        if path.exists():
            ...
            if entry["package_sha256"] != bundle.p.sha256 or entry.get("protocol_version") != "v2.1":
                raise ValueError("Cannot mix source package/edition identities in a figure directory")
            all_entries.append(entry)
    rows = []
    markdown = ["# Protocol v2.1 figure index", "", "Proposed method: F04. Primary evaluation point: v3. MFIG21 retains its original v2 artifact and is excluded from the 28 reissued figures.", "",
    ...
    for entry in all_entries:
        ...
        fid = entry["figure_id"]
        rows.append({"figure_id": fid, "title": entry["title"], "kind": entry["kind"], "protocol_version": "v2.1",
    ...
    (out / "GALLERY.html").write_text('<!doctype html><html lang="en"><meta charset="utf-8"><title>Protocol v2.1 figures</title><style>body{font:16px sans-serif;margin:2rem auto;max-width:1100px;color:#222}article{margin-bottom:3rem}img{max-width:100%;height:auto}p{line-height:1.5}</style><h1>Protocol v2.1 figures</h1>'+"".join(cards)+"</html>", encoding="utf-8")
    ...
    result = {"status": "COMPLETE" if len(all_entries) == len(FIGURES) and not failures else "INCOMPLETE",
              ...
              "protocol_version": "v2.1", "MFIG21_policy": "unchanged original v2 artifact; excluded from v21 registry",
              ...
              "count_semantics": "The v2.1 registry contains exactly 28 figures, excluding unchanged MFIG21. Edition origins partition all composites: reissued + new = rendered. Supplementary is an overlapping publication role: SFIG01 reissued, SFIG02 new.",
