"""P-13 native execution with explicit parameter and provider identity ledgers.

Reuses the P-09c scheduler, native output checks and failure classification.
The native kernel changes only through the separately bridged F02 guard.
"""
...
from ..clean5_parity.runtime import ALLOWED
...
def patch_config(original, replacements, model, *, sensor_corrections=True,
                 apply_calibration=True, parameter_overrides=None):
    """Token-preserving parameter binding with an unchanged hash definition."""
    bound, _ = bind_config(original, replacements)
    before = yaml.safe_load(original)
    updates = dict(parameter_overrides or {})
    if set(updates)-{'vrw','abstd'}:
        raise ValueError('Only preregistered grid vrw/abstd overrides are allowed')
    if apply_calibration:
        updates.update({k: model[k] for k in ('vrw', 'abstd')})
    if sensor_corrections:
        updates['basic_dual_yaw_fixed_std_deg'] = 2.933193
    lines, ledger = [], []
    for line in bound.splitlines(keepends=True):
        key = line.split(':', 1)[0]
        if key in updates:
            line = key+': '+json.dumps(updates[key], allow_nan=False)+('\n' if line.endswith('\n') else '')
        lines.append(line)
    text = ''.join(lines)
    after = yaml.safe_load(text)
    if set(before) != set(after) or not set(updates) <= set(before):
        raise ValueError('P13 template field set mismatch')
    allowed = set(ALLOWED) | set(updates)
    differences = {k for k in before if before[k] != after[k]}
    if differences - allowed:
        raise ValueError('Unregistered runtime difference '+str(differences-allowed))
    for key in ('arw', 'gbstd', 'initbastd', 'corrtime', 'imu_install'):
        if key in before and before[key] != after[key]:
            raise ValueError('Frozen native parameter changed '+key)
    for k in sorted(differences):
        ledger.append({'key': k, 'before': before[k], 'after': after[k], 'scientific_parameter': k not in ALLOWED})
    return text, {'changed_keys': sorted(differences), 'parameter_diff': ledger,
        'scientific_parameter_changed_keys': sorted(differences-set(ALLOWED)),
        'frozen_parameter_hash': frozen_parameter_hash(text),
        'reference_frozen_parameter_hash': frozen_parameter_hash(original),
        'frozen_parameter_hash_definition_unchanged': True,
        'separate_sensor_group': 'SENSOR_MODEL_V21', 'per_case_tuning': False}

def run_one(source, bundle, contract, reg, run_root, code_commit, *, original_text=None,
            dataset='BY2', sequence_spec=None, case_meta=None, sensor_corrections=True,
            apply_calibration=True, parameter_overrides=None):
    root = Path(run_root)
    ...
    record = {**FLAGS, 'protocol_id': PROTOCOL, 'chain': 'V21', 'dataset_id': dataset,
              ...
              'data_mode': 'semisynthetic' if source['case_id'].startswith('D') else 'real_clean',
              'semisynthetic_data_used': source['case_id'].startswith('D'),
              'sensor_model_hash': contract.get('sensor_model_hash'),
    ...
    try:
        ...
        text, diff = patch_config(original_text, replacements, model,
            sensor_corrections=sensor_corrections, apply_calibration=apply_calibration,
            parameter_overrides=parameter_overrides)
        v2_reference,_=patch_config(original_text,replacements,model,
            sensor_corrections=False,apply_calibration=apply_calibration,
            parameter_overrides=parameter_overrides)
        if dataset != 'BY2':
            text, role_audit = append_sequence_runtime_role(text, contract['restart_authorization']['sequence_runtime_role'])
            v2_reference,_=append_sequence_runtime_role(v2_reference,contract['restart_authorization']['sequence_runtime_role'])
            record['runtime_role_transport_audit'] = role_audit
        diff['before_transport_frozen_parameter_hash']=diff['frozen_parameter_hash']
        diff['frozen_parameter_hash']=frozen_parameter_hash(text)
        diff['v2_cal_reference_frozen_parameter_hash']=frozen_parameter_hash(v2_reference)
        reference_payload,actual_payload=yaml.safe_load(v2_reference),yaml.safe_load(text)
        correction_keys=sorted(k for k in reference_payload if reference_payload[k]!=actual_payload[k])
        if correction_keys != (['basic_dual_yaw_fixed_std_deg'] if sensor_corrections else []):
            raise ValueError('Native difference against same CAL/variant reference is not the registered yaw std')
        diff['v2_to_v21_scientific_parameter_changed_keys']=correction_keys
        cfg = yaml.safe_load(text)
        if [cfg['starttime'], cfg['endtime']] != spec['window_seconds']:
            raise ValueError('Frozen sequence window changed')
        cfg_path = root/'PROTOCOL_V21_RUNTIME_CONFIG.yaml'
        ...
        if result.returncode != 0:
            ...
            # Human adjudication: metadata/counter-mechanism conflicts are logged,
            # never re-execute an already completed native run to repair them.
            record['bookkeeping_notes'] = []
            for label, check in (
                ('native_identity_audit', lambda: validate_identity(native, cfg, native_source, bundle)),
                ('counter_audit', lambda: check_counters(native, cfg, expected)),
                ('auxiliary_counter_audit', lambda: check_auxiliary(native, cfg, expected))):
                try:
                    record[label] = check()
                    if record[label].get('pass', record[label].get('passed', True)) is False:
                        record['bookkeeping_notes'].append({'check': label, 'disposition': 'RETAIN_ACTUAL_COUNTERS_FOR_REVIEW', 'evidence': record[label]})
                except (ValueError, KeyError, TypeError) as error:
                    record[label] = {'status': 'VALIDATION_MECHANISM_ADAPTATION_REQUIRED', 'reason': str(error)}
                    record['bookkeeping_notes'].append({'check': label, 'reason': str(error)})
            record['counters'] = record.get('counter_audit', {}).get('actual', {})
            record['native_counters_full'] = native
    ...
    write_json(root/'P13_RUN_TERMINAL.json', record)
