"""P-04 eight-run, two-sequence frozen-binary execution; no retry or prior-run mutation."""
...
def run_order(contract, dataset=None):
    expected=[(d,v,m) for d in ('BY2H','BY2O') for v,m in
              (('V1','A04'),('V2','A04'),('V2is','A04'),('V2is','F03'))]
    actual=[tuple(row) for row in contract['p04']['run_order']]
    if actual!=expected:raise ValueError('P04 requires exactly the eight preregistered serial runs')
    if dataset is not None:
        if dataset not in ('BY2H','BY2O'):raise ValueError('Invalid P04 dataset')
        return [(v,m) for d,v,m in actual if d==dataset]
    return actual


def verify_bundle(bundle):
    if set(bundle['variants'])!={'V1','V2','V2is'}:raise ValueError('P04 provider variants differ')
    for value in bundle['variants'].values():
        if set(value['providers'])!={'imupath','gnsspath','raw_doppler_factor_path','go2_attitude_prior_path','go2_horizontal_velocity_prior_path'}:
            raise ValueError('P04 provider path roles differ')
        for entry in value['providers'].values():
            path=Path(entry['path'])
            if path.is_symlink() or sha256_file(path)!=entry['sha256']:raise ValueError('P04 provider hash mismatch')


def run_ladder(*,registry,stage_root,contract,provider_bundle,executable,code_commit,dataset):
    order=run_order(contract,dataset);verify_bundle(provider_bundle)
    spec=contract['p04']['sequences'][dataset]
    if provider_bundle['dataset_id']!=dataset:raise ValueError('Provider dataset mismatch')
    ...
    for variant,method in order:
        run_id=f'CLEAN5_PARITY_{dataset}_{variant}_{method}'
        root=runs_root/run_id;root.mkdir()
        source=spec['original_configs'][method]
        ...
        record={'variant_id':variant,'provider_family':provider_bundle['variants'][variant]['provider_family'],'method_id':method,'run_id':run_id,'terminal_status':'NOT_STARTED',
                ...
                'output_root':str(root),'dataset_id':dataset,'case_id':f'CLEAN5_{dataset}_NATURAL','data_mode':registry.sequences[dataset].data_mode,
        ...
        try:
            result=run_process_group(command,cwd=registry.code_root,timeout_seconds=1800,
                      timeout_message='P04 solver timeout',launch_failure_message='P04 solver launch failed')
