"""Faithful non-official dual-antenna heading-aided EKF."""
...
def run_method(epochs: list[ProviderEpoch]) -> list[EpochOutput]:
    ...
    p = np.diag([2.0, 2.0, 1.0, 4.0, 4.0, 1.0, math.radians(20.0) ** 2])
    ...
    for epoch in epochs:
        ...
        q_scale = max(dt, 0.1)
        q = np.diag([0.08, 0.08, 0.03, 0.4, 0.4, 0.08, math.radians(3.0 * q_scale) ** 2])
        p = f @ p @ f.T + q
        updates: list[str] = []
        if epoch.valid_position:
            z = np.array([epoch.pos_n_m, epoch.pos_e_m, epoch.pos_u_m, epoch.vel_n_mps, epoch.vel_e_mps, epoch.vel_u_mps])
            h = np.zeros((6, 7))
            for i in range(6):
                h[i, i] = 1.0
            r = np.diag([
