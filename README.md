...
After `N9G1C_TO_N9G1E_PROVIDER_CONTRACT_RESOLUTION_ACTIVE_FGO_BACKEND_AND_NORMAL_SMOKE` and `BY3A3_SELECTED_FEEDBACK_STAGE1_CHAIN_AND_NORMAL_GENERALIZATION_EXECUTION`.
...
For BY3, run `BY3B_DEGRADATION_MATRIX_PLANNING_AND_PRECHECK` before any BY3 degradation execution. For active FGO, implement the active FGO backend or reframe scope before any N9G2 representative validation.
...
The BY3 decision is `BY3A3_normal_generalization_completed`, setting `ready_for_BY3_degradation_matrix_planning=true` for planning/precheck while keeping `ready_for_paper_claims=false`.
