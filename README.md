...
XB1 / PG1_20260105_122513 has also completed the first poor-GNSS repeated-experiment bootstrap, blocker triage, and A1 relpos-difference correction audit. XB1 GNSS quality is classified severe. Body IMU was generated from `<XB1_BODY_SOURCE>`, Go2 priors were materialized, and XB1A1 repaired Raw Doppler provider materialization through the accepted RTKLIB/RINEX/helper path using WSL gcc/helper execution. XB1A2 then corrected the A1 provenance question: the BY2/process_data-compatible status `rel_pos_gnss2-rel_pos_gnss1` path and the BY3A5B absolute-position repair path were both audited for XB1, and both remain nonphysical. `LegSA_full_EKF` and `final_v23_dual_antenna_EKF` normal runs remain blocked/not applicable; `single_antenna_gnss1_status_KF_GINS` completed normal official evaluation earlier as a diagnostic baseline only. No degradation, retuning, quality-aware execution, paper claims, or poor-GNSS robustness claims are authorized.
...
N9G0A resolved the historical Git boundary. PR #52 remains open/unmerged unless the human explicitly approves merge, closure, or tag actions.
...
BY3A3 repaired the same-case selected-feedback chain. It ran a BY3 stage1 baseline, ran official evaluation, generated BY3 same-case feedback from the stage1 official-eval state/estimate table only, then ran BY3 normal-only `LegSA_full_EKF`, GNSS1-status single baseline, and final_v23 external baseline official evaluations. BY3A3 did not run BY3 degradation, artificial degradations, parameter retuning, trace tuning, final_v23 solver input for LegSA, BY2 feedback reuse, or paper-claim work.
...
BY3A4C recovered the historical BY2/N4 yaw-reference repair from git/docs/runtime evidence. N4H2 old yaw around 93 deg was invalidated by N4H2D; N4H2D selected `official_ref_sign_minus` and fresh replay yaw was about 1.98 deg against the reconstructed dual official reference. BY3A5 then confirmed the old BY3 dual-yaw input was wrong-source, but its HDT replacement policy is now superseded. BY3A5B regenerated BY3 dual yaw from GNSS1/GNSS2 A1_dual_diff short-baseline positions with BY2 lateral conversion and fixed_1p5 yaw_std. BY3A6 locked the trace truth reference, validated evaluator field selection/base_time, repaired stale first-row initatt for stage1/LegSA, and reran BY3 normal only. BY3A7 then confirmed the remaining yaw failure was caused by BY3 Go2 IMU gyro bias estimated from a moving segment after selected Go2 start, repaired a BY3A7-local pre-motion source-bias IMU input, and reran BY3 normal only. BY3A8 budgeted the remaining yaw error and found the A1 observation lower bound is poor: A1-vs-trace heading RMSE is about 24.06 deg and p95 about 31.78 deg. BY3B completed position/up degradation planning with diagnostic yaw only. BY3C completed the approved Batch0-Batch3 position/up degradation execution subset only. BY3C1/BY3Y1 reviewed the existing BY3C results, generated position/up review tables and figures from existing metrics, compared overlapping BY2/BY3 families, and explained the BY3 yaw limitation as diagnostic-only. GEN1 then built BY2/BY3 three-scheme inventories and cross-dataset tables from existing metrics, generated cross-dataset figures from existing metrics only, and created a copy-only BY3 figure organization. No new solver/evaluator/degradation/random-generation work or paper claims were authorized.

Next planned stage:

For BY3, review `GEN1_BY2_BY3_GENERALIZATION_REPORT_AND_BY3_FIGURE_ORGANIZATION` before any later mixed position/up planning, diagnostic-yaw planning, or other-dataset planning. For XB1, review `XB1A2_A1_DUAL_DIFF_RELPOS_DIFFERENCE_REAUDIT_AND_NORMAL_RERUN` before deciding whether to plan a separate quality-aware diagnostic branch or inspect PG2 source quality. For active FGO, implement the active FGO backend or reframe scope before any N9G2 representative validation.
...
N8A2 fixes the FGO yaw residual convention by using shortest-angle wrapping for
dual-yaw, yaw smoothness, and yaw-rate diagnostic residuals. Smoothness is
...
BY3A5 correctly audited that the old BY3 15-column yaw input was wrong-source. Its HDT repaired-input policy is diagnostic/rejected/superseded for mainline BY3 and must not be reused as solver yaw input.

BY3A5B repairs the mainline BY3 dual yaw with A1_dual_diff short-baseline yaw from GNSS1/GNSS2 absolute positions, using BY2 `gnss2_minus_gnss1`, lateral conversion equivalent to `baseline_heading+90`, and fixed_1p5 yaw_std. GNSS status long-baseline `rel_pos_n/e/d` and NMEA HDT are rejected as mainline solver yaw sources. BY3A6 then validated trace truth/evaluator/base_time, confirmed stage1/LegSA stale first-row initatt, repaired initatt to use the first A1 yaw at or after requested starttime, and completed a normal-only rerun. BY3A7 repaired the BY3 Go2 IMU moving-segment gyro-bias preprocessing issue with a BY3A7-local pre-motion source-bias IMU and completed a normal-only rerun. BY3A8 found the remaining 4-5 deg dual-yaw output error is limited by A1 observation quality: A1-vs-trace heading RMSE is about 24.06 deg, p95 about 31.78 deg, and no source-backed additional repair passed. BY3B locked future degradation planning as position/up primary with diagnostic yaw only. BY3C executed only the approved position/up Batch0-Batch3 subset; paper claims remain disabled.
