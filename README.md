...
After `N9G1C_TO_N9G1E_PROVIDER_CONTRACT_RESOLUTION_ACTIVE_FGO_BACKEND_AND_NORMAL_SMOKE` and `BY3C1_TO_BY3Y1_POSITION_UP_GENERALIZATION_REVIEW_AND_YAW_DIAGNOSTIC_EXPLANATION`.
...
The BY3 decision is `BY3C1_position_up_review_and_yaw_diagnostic_complete`, setting `ready_for_BY3D_or_other_dataset_planning=true` only after human review, `yaw_claim_scope=diagnostic_only`, and `ready_for_paper_claims=false`.
