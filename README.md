...
The BY3 decision is `BY3A5B_a1_dual_diff_input_repaired_but_yaw_reference_issue_remains`, setting `ready_for_BY3_degradation_matrix_planning=true` only for `position_up_only`, `yaw_degradation_claims=false`, and `ready_for_paper_claims=false`.
...
BY3A5B repairs the mainline BY3 dual yaw with A1_dual_diff short-baseline yaw from GNSS1/GNSS2 absolute positions, using BY2 `gnss2_minus_gnss1`, lateral conversion equivalent to `baseline_heading+90`, and fixed_1p5 yaw_std. GNSS status long-baseline `rel_pos_n/e/d` and NMEA HDT are rejected as mainline solver yaw sources. The BY3A5B normal-only rerun completed; official yaw still indicates a reference/evaluator issue, so only position/up BY3 planning is ready and paper claims remain disabled.
