...
- Current BY3/reporting stage: `BY3A5B_A1_DUAL_DIFF_YAW_INPUT_REPAIR_AND_NORMAL_RERUN`; A1 dual-diff yaw input repaired from GNSS1/GNSS2 short baseline, normal rerun completed, official yaw reference issue remains, and position/up-only planning is allowed for human review.
...
- BY3A5B superseded BY3A5's HDT policy, reconstructed BY3 A1_dual_diff yaw from GNSS1/GNSS2 short-baseline absolute positions, rejected status long-baseline `rel_pos_n/e/d`, generated a fixed_1p5 repaired 15-column GNSS input, reran BY3 normal only, and concluded that yaw input is repaired but official yaw reference/evaluator remains unresolved.
...
- `human_review_yaw_reference_or_position_only_BY3B`
- BY3 yaw reference confirmation before any BY3 yaw degradation planning or yaw claim
...
BY3A5B is the accepted mainline input repair: BY3 dual yaw is regenerated from GNSS1/GNSS2 A1_dual_diff short-baseline absolute positions with BY2 sign/lateral conversion and fixed_1p5 yaw_std. Status long-baseline `rel_pos_n/e/d` and NMEA HDT are rejected as mainline solver yaw sources. BY3A5B normal-only rerun completed, but official yaw remains a reference/evaluator issue. `ready_for_BY3_degradation_matrix_planning=true` only for `position_up_only`, `yaw_degradation_claims=false`, and `ready_for_paper_claims=false`.
