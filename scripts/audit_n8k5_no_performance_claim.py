#!/usr/bin/env python3
"""Audit N8K5 makes no paper performance claim."""

# 中文说明：N8K5 输出只用于工程绘图审计，不形成论文性能结论。
...
def _audit(root: Path) -> int:
    for name in REQUIRED_REPORTS:
        ...
        if payload.get("paper_performance_claim") is not False or payload.get("outperform_final_v23_claim") is True:
            raise SystemExit(f"audit_n8k5_no_performance_claim failed: {name}")
    print("audit_n8k5_no_performance_claim passed")
    return 0
