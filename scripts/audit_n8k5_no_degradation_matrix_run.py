#!/usr/bin/env python3
"""Audit N8K5 did not run the degradation matrix."""

# 中文说明：N8K5 不进入 N9B 全量退化矩阵。
...
def _audit(root: Path) -> int:
    for name in REQUIRED_REPORTS:
        ...
        if payload.get("degradation_matrix_run") is not False:
            raise SystemExit(f"audit_n8k5_no_degradation_matrix_run failed: {name}")
    print("audit_n8k5_no_degradation_matrix_run passed")
    return 0
