...
def _make_review(path: Path, input_report: dict[str, Any], yaw_report: dict[str, Any], source_map: dict[str, Any]) -> None:
    lines = [
        ...
        f"- reconstructed_process_data_compat_coverage_status: {input_report.get('reconstructed_process_data_compat_coverage_status')}",
        f"- coverage_suspicious: {str(input_report.get('coverage_suspicious')).lower()}",
        "",
