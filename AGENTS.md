...
HX-02D（2026-09-26，只读诊断）：起点 e612eeb，HX02 报告副本一致、无需补提交；四运行证据更支持生产移植/配置中的接触更新与零偏反馈（H3），具体公式/代码缺陷不能判定，不支持简单镜像或 yaw 反号为主因；原生 LegSA/外部 0/0，参考子进程 2 次且各 trace 只读打开 1 次，B 主诊断 3 次（首个 JSON 整数序列化失败保留）及无参考状态分解 1 次；封存表续作开始/提交前均 65/65、CSV 59/59，HX02 全目录 12678/12678、方法本体 423/423、既有未跟踪 29/29 不变，scratch 清空，不做交接包；记录 docs/paper_rebuild/hext/HX02D/HX02D_HARTLEY_DIAGNOSTIC.md。
HX-02E（2026-09-26，官方 Hartley）：登记 1fc59f6067ddd326b578a7a3a47e62138ae3abe4；RossHartley/invariant-ekf@ef16e8a 原仓库前后干净、58/58 文件及构建副本相同；OFF-LIT/OFF-DEF × BY2/BY2H/BY2O 六原生/六相对评估全部 COMPLETED，LegSA 解算/评估 0/0，六子进程各 trace 只读打开 1 次且核哈希；示例 2、合成门 4 另计，首次后台启动在脚本打开前中断、无真实调用重试；65 pin 开始及两次提交前均 65/65（CSV 59/59），HX02 12678、HX02D 82、方法本体 423、既有未跟踪 29 均不变；官方两配置无门槛作为该类比较，旧移植与诊断退补充；96 行表及审计见 docs/paper_rebuild/hext/HX02E/HX02E_RESULTS.md，scratch 清空、无交接包。

HX-03（2026-09-26）：登记 13d8553ccfd4862284dd54f5fb392f92e99fd2f5、调度记录修正 ff7178429f4cda93fe2c2124e8a18a8555e782b9；LC01/EXT05C Classic-18 与 A2、LC01-BR（改动过的 LC01，仅 A2 补充）完成；LegSA 0/0，外部原生 285（身份门 3）、评估 492（身份门 4），重试 0；381 目录归档，396 行报告中 214 可用、36 原生发散、146 D12 不可用（含 A1 等价 18 行）；A2 BR 有限 0/18，精度改善不能判定；65 pin 两次 65/65、方法本体 423/423、输入 397/397、既有未跟踪 29/29；每评估参考只读打开并核哈希各 1 次，原生/控制器参考 0，线程白名单均 1；表全文与审计 docs/paper_rebuild/hext/HX03/，scratch 清空、无交接包。
HX-03R-1（2026-09-26，只读诊断）：146 个 D12 不可用逻辑行/113 个物理目录与 20 个随机 v3 D03/D04 对照已登记；确认 HX-03 球面近似观察器与冻结 WGS84 投影不一致，对照采用另一 WGS84 策略；因审计残差/argmax 未留存，固定时间偏差与全部行的唯一归因不能判定，原结果保持不可用；原生/评估 0/0、参考打开 0，诊断进程 1；65 pin 两次 65/65（CSV 59/59）、方法本体 423/423、既有未跟踪 29/29 一致；14505 个 HX03 既存文件元数据未变，1079 项所读输入哈希复核一致；报告 docs/paper_rebuild/hext/HX03R/HX03R1_D12_DIAGNOSIS.md，逐例表完整保留于 G: 与仓库。
HX-03R-2（2026-09-26）：登记 24adc4385d926a572ec809d010e065fa22cf5a5b；采用 v3 既有 WGS84 观察器，492 次重评均通过原 D12，984 份冻结科学输出与 HX-03 字节一致，旧可用指标相等；146 行由审计不可用转有限，360/396 有限、36 原生发散、0 审计不可用，A1/A2 18 对相等；原生 0、LegSA 0/0，492 次参考只读打开并核哈希各 1 次；65 pin 三次 65/65、方法本体 423/423、既有未跟踪 29/29，HX-03 14525 项既存元数据不变；首次 Python 执行前 strace 失败单列保留，科学重试 0；逐例表、残差与证据 docs/paper_rebuild/hext/HX03R2/，scratch 清空、无交接包。
HX-05（2026-09-26）：登记 0ebb341dcf968aca71ce7fe057204794935ac1d9；按 HX-02D C4 完整公式完成 LEG-DR 三序列积分/相对评估 3/3，BY2 逐历元一致、指标最大差 1.71e-9 < 1e-6；LegSA 解算/评估 0/0、外部解算 0，三次参考各只读打开并核哈希一次；三张手稿表、BY2O 分段表和三图完成，机器 QA 25/25、实际看图 3/3；九行退化主表之外的 D43 单列补充，不改族定义；65 pin 三次 65/65、方法本体 423/423、既有未跟踪 29/29，前阶段未改；结果 docs/paper_rebuild/hext/HX05/，逐文件归档复核、scratch 清空、无交接包。
